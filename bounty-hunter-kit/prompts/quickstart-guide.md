...
node scripts/bounty-claude-builders.js    # claude-builders 美元悬赏
node scripts/opire-bounty-scout.js        # Opire 平台美元悬赏
...
3. `opire-bounty-scout.js` — Opire 平台，$50-$500
4. `rustchain-bounty-scout.js` — 代币悬赏，5 RTC 起
...
# 只看 claude-builders 美元悬赏
node scripts/bounty-claude-builders.js

# 只看 Opire 平台美元悬赏
node scripts/opire-bounty-scout.js

# POIDH 新手入门（Albums Bug Fix 系列）
node scripts/poidh-app-scout.js
...
```
第1天 → POIDH Albums Bug Fix（熟悉流程）
第2天 → SolFoundry T1（赚 FNDRY）
第3天 → Opire/claude-builders（赚美元）
第4天+ → 挑战高价值 Bounty（$200+）
```
